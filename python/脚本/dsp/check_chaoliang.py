...
def get_order_impression(date):
    ...
    count =  cursor.execute("SELECT  order_id,impression,status FROM  dsp.dsp_date_configs daily  left join dsp.dsp_orders o on (o.id = daily.order_id)  WHERE date =  '%s' AND solution_id = 0 "  % date )
